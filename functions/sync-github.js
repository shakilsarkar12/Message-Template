exports.handler = async function (event, context) {
  ...
  if (!token || !repo) {
    return {
      statusCode: 500,
      body: JSON.stringify({
        error: "Missing GITHUB_TOKEN or GITHUB_REPO in Netlify Environment Variables. Please set GITHUB_TOKEN and GITHUB_REPO in Netlify Dashboard."
      })
    };
  }
  ...
  try {
    ...
    if (!getRes.ok) {
      ...
      return {
        ...
        body: JSON.stringify({ error: err.message || `Could not fetch ${path} from ${repo}. Check repository name & token permissions.` })
      };
    }
